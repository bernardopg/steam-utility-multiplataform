...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `check_ownership` | `<output_path> [app_ids]` | `<output_path> [app_ids_json_or_file]` | Command exists and writes upstream-style `games.json` payload | Usage/failure/success file output is regression-tested; Linux live validation exists for explicit AppIDs; default remote app-id source is still untested | Yes for true ownership lookup | deterministic CLI tests added + fake-backed native seam + Linux real-environment validation completed + Windows real-environment validation pending | Add default-source test seam and Windows real-session validation |
...
3. coverage for the default remote AppID source path used by `check_ownership`
4. release-workflow/test-harness hardening around the custom test runner
